...
def style_button_rects(game: Any) -> dict[str, pygame.Rect]:
    ...
    return {
        ...
        ArtStyle.ILLUSTRATED.value: pygame.Rect(
            left + button_width + gap, top, button_width, 38
        ),
    }
...
def draw_start_menu(
    ...
    for style_name, rect in style_button_rects(game).items():
        ...
        fill = (
            (18, 23, 35)
            if pressed_button == f"style:{style_name}" and hovered
            else (47, 56, 76) if hovered or selected else (27, 33, 48)
        )
        pygame.draw.rect(screen, fill, rect, border_radius=7)
        pygame.draw.rect(
            screen, accent, rect, 3 if selected or hovered else 1, border_radius=7
        )
        label = body_font.render(
            style_name.upper(), True, game.WHITE if selected else game.MUTED
        )
        screen.blit(
            label,
            (
    ...
    for mode, rect in mode_button_rects(game).items():
        ...
        fill = (
            (18, 23, 35)
            if pressed_button == mode and hovered
            else (39, 47, 65) if hovered else (27, 33, 48)
        )
        ...
        screen.blit(
            heading_surface,
            (rect.centerx - heading_surface.get_width() // 2, rect.y + 7),
        )
        screen.blit(
            detail_surface,
            (rect.centerx - detail_surface.get_width() // 2, rect.y + 29),
        )
...
def draw_discovered_walls(
    screen: pygame.Surface, state: Any, game: Any, reveal_all: bool = False
) -> None:
    ...
    for wx, wy in horizontal_walls:
        ...
        pygame.draw.line(
            screen, WALL, (wx * TILE_SIZE, py), ((wx + 1) * TILE_SIZE, py), 6
        )
    for wx, wy in vertical_walls:
        ...
        pygame.draw.line(
            screen,
            WALL,
            (px, HUD_HEIGHT + wy * TILE_SIZE),
            (px, HUD_HEIGHT + (wy + 1) * TILE_SIZE),
            6,
        )
...
def draw_classic_game(
    ...
    for scorch_x, scorch_y in state.scorched:
        scorch_rect = (
            scorch_x * TILE_SIZE + 2,
            HUD_HEIGHT + scorch_y * TILE_SIZE + 2,
            TILE_SIZE - 4,
            TILE_SIZE - 4,
        )
        ...
        pygame.draw.line(
            screen,
            (120, 55, 48),
            (scorch_rect[0] + 10, scorch_rect[1] + 13),
            (scorch_rect[0] + 29, scorch_rect[1] + 30),
            2,
        )
        pygame.draw.line(
            screen,
            (120, 55, 48),
            (scorch_rect[0] + 35, scorch_rect[1] + 12),
            (scorch_rect[0] + 25, scorch_rect[1] + 37),
            2,
        )
    ...
    if not state.has_treasure:
        ...
        pygame.draw.rect(
            screen, (115, 60, 43), (tx - 16, ty - 2, 32, 17), border_radius=3
        )
        pygame.draw.polygon(
            screen,
            (151, 84, 50),
        ...
        pygame.draw.line(
            screen, (255, 239, 164), (tx + 16, ty - 17), (tx + 16, ty - 7), 1
        )
        pygame.draw.line(
            screen, (255, 239, 164), (tx + 11, ty - 12), (tx + 21, ty - 12), 1
        )
        ...
        pygame.draw.ellipse(screen, (18, 24, 38), (tx - 18, ty + 12, 36, 8))
        pygame.draw.rect(
            screen, (115, 60, 43), (tx - 16, ty + 2, 32, 13), border_radius=3
        )
        pygame.draw.polygon(
            screen,
            (151, 84, 50),
    ...
    if debug or state.dragon_awake or state.status != "playing":
        ...
        pygame.draw.polygon(
            screen, (255, 157, 66), [(dx - 7, dy - 5), (dx, dy - 14), (dx + 7, dy - 5)]
        )
        pygame.draw.polygon(
            screen, RED, [(dx - 11, dy - 7), (dx - 7, dy - 17), (dx - 2, dy - 7)]
        )
        pygame.draw.polygon(
            screen, RED, [(dx + 2, dy - 7), (dx + 7, dy - 17), (dx + 11, dy - 7)]
        )
        pygame.draw.polygon(
            screen,
            RED,
        ...
        if state.dragon_awake:
            ...
            pygame.draw.line(
                screen, (35, 15, 24), (dx - 9, dy - 4), (dx - 3, dy - 4), 2
            )
            pygame.draw.line(
                screen, (35, 15, 24), (dx + 3, dy - 4), (dx + 9, dy - 4), 2
            )
        ...
        pygame.draw.rect(
            screen, (111, 37, 54), (dx - 12, dy + 13, 8, 6), border_radius=2
        )
        pygame.draw.rect(
            screen, (111, 37, 54), (dx + 4, dy + 13, 8, 6), border_radius=2
        )
    ...
    if debug and phantom_state is not None and phantom_state.dragon_awake:
        ...
        center = TILE_SIZE // 2
        ghost_color = (
            (129, 220, 255, 145) if phantom_state.hard_mode else (190, 145, 255, 145)
        )
        pygame.draw.polygon(
            ghost,
            ghost_color,
    ...
    for y in range(GRID_SIZE):
        for x in range(GRID_SIZE):
            ...
            hidden = (x, y) not in state.visited and max(
                abs(x - player_x), abs(y - player_y)
            ) > VISION_RADIUS
    ...
    draw_discovered_walls(
        screen, state, game, reveal_all=debug or state.status != "playing"
    )
    ...
    if state.status == "won":
        headline, detail, color = (
            "ESCAPED!",
            f"Treasure secured in {state.moves} moves · press R",
            GREEN,
        )
    elif state.status == "escaped":
        headline, detail, color = (
            "SAFE FOR NOW",
            "You escaped through the portal · press R to try again",
            BLUE,
        )
        ...
        if now < state.flash_until:
            ...
            distance = max(
                abs(state.player[0] - state.dragon[0]),
                abs(state.player[1] - state.dragon[1]),
            )
            detail = (
    ...
    menu_fill = (
        (18, 23, 35)
        if pressed_button == "main_menu" and menu_hovered
        else (47, 56, 76) if menu_hovered else (31, 38, 55)
    )
    pygame.draw.rect(screen, menu_fill, menu_button, border_radius=7)
    pygame.draw.rect(
        screen,
        WHITE if menu_hovered else MUTED,
        menu_button,
        3 if menu_hovered else 2,
        border_radius=7,
    )
...
def draw_illustrated_game(
    ...
    def tile_hidden(position: tuple[int, int]) -> bool:
        ...
        return (
            ...
            and position not in state.visited
            and outside_vision
        )
    ...
    def blit_at_cell(image: pygame.Surface | None, position: tuple[int, int]) -> None:
        ...
        screen.blit(
            image,
            (center_x - image.get_width() // 2, center_y - image.get_height() // 2),
        )
    ...
    if all(floors):
        for y in range(game.GRID_SIZE):
            for x in range(game.GRID_SIZE):
                screen.blit(
                    floors[(x + y) % 2],
                    (x * tile_size, game.HUD_HEIGHT + y * tile_size),
                )
    ...
    if not tile_hidden(state.treasure):
        if (
            not state.has_treasure
            or pygame.time.get_ticks() < state.treasure_open_until
        ):
            treasure = assets.get(
                ArtStyle.ILLUSTRATED, "treasure_open", floor_size, fit=True, trim=True
            )
    ...
    if debug or state.dragon_awake or state.status != "playing":
        ...
        dragon = assets.get(
            ArtStyle.ILLUSTRATED, dragon_name, floor_size, fit=True, trim=True
        )
        blit_at_cell(dragon, state.dragon)

    if debug and phantom_state is not None and phantom_state.dragon_awake:
        phantom = assets.get(
            ArtStyle.ILLUSTRATED, "dragon/phantom", floor_size, fit=True, trim=True
        )
    ...
    for wall_x, wall_y in horizontal_walls:
        if horizontal is not None:
            screen.blit(
                ...
                (
                    wall_x * tile_size,
                    game.HUD_HEIGHT + (wall_y + 1) * tile_size - wall_offset,
                ),
            )
    for wall_x, wall_y in vertical_walls:
        if vertical is not None:
            screen.blit(
                vertical,
                (
                    (wall_x + 1) * tile_size - wall_offset,
                    game.HUD_HEIGHT + wall_y * tile_size,
                ),
            )
