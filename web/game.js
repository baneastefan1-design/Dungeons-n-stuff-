(() => {
  ...
  const menuTitle = document.querySelector('#menu-title'), menuCopy = document.querySelector('#menu-copy'), dismissResult = document.querySelector('#result-dismiss'), nextButton = document.querySelector('#new-game'), soundButton = document.querySelector('#sound-toggle'), profileButton = document.querySelector('#profile-toggle'), usernameInput = document.querySelector('#username'), debugLevelField = document.querySelector('#debug-level-field'), debugLevelInput = document.querySelector('#debug-level'), leaderboard = document.querySelector('#leaderboard'), leaderboardBody = document.querySelector('#leaderboard-body'), assets = {};
  ...
  const debugMode = location.pathname === '/debug';
  ...
  if (!clientId) { clientId = crypto.randomUUID(); localStorage.setItem('dungeon-escape-client-id', clientId); }
  usernameInput.value = localStorage.getItem('dungeon-escape-username') || '';
  if (debugMode) { debugLevelField.hidden = false; document.querySelector('.eyebrow').textContent = 'DUNGEON ESCAPE · DEBUG MODE'; }
  ...
  function showMainMenu() { lastResult = null; dismissResult.hidden = true; menuTitle.textContent = debugMode ? 'Dungeon Escape debug' : 'Dungeon Escape'; menuCopy.textContent = debugMode ? 'Choose a level, reveal the entire dungeon, and compare the opposite dragon as a ghost.' : 'Find the treasure, evade the sleeping dragon, and return to the portal.'; menu.hidden = false; nextButton.textContent = 'Main menu'; }
  function connect() { if (socket?.readyState === WebSocket.OPEN) return; socket = new WebSocket(`${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.host}/ws${debugMode ? '?debug=1' : ''}`); socket.onmessage = event => { const next = JSON.parse(event.data); if (next.error) { menuCopy.textContent = next.error; menu.hidden = false; return; } const previous = state; state = next; playFeedback(previous, state); status.textContent = state.hint; status.hidden = state.status === 'playing'; if (state.status !== 'playing' && !resultTimer) { const [title, copy] = resultCopy(state.status); lastResult = {title, copy}; resultTimer = setTimeout(() => { showResult(); resultTimer = null; }, 2000); } draw(); }; socket.onclose = () => { status.hidden = false; status.textContent = 'Connection lost. Reload to reconnect.'; }; }
  function start(difficulty) { const username = usernameInput.value.trim().replace(/\s+/g,' '); if (!username) { menuCopy.textContent = 'Enter an explorer name to start your ranked run.'; usernameInput.focus(); return; } wakeAudio(); localStorage.setItem('dungeon-escape-username', username); clearTimeout(resultTimer); resultTimer = null; lastResult = null; state = null; dismissResult.hidden = true; nextButton.textContent = 'Main menu'; connect(); const begin = () => send({action:'start', difficulty, username, client_id: clientId, level: debugMode ? Number(debugLevelInput.value) : undefined}); if (socket.readyState === WebSocket.OPEN) begin(); else socket.addEventListener('open', begin, {once:true}); menu.hidden = true; }
  ...
  function draw() {
    ...
    const threat = state.forced_hard ? 'Hard instincts' : (state.hard_mode ? 'Hard' : 'Normal'); ctx.fillStyle='#f6f4e8'; ctx.font=`700 ${Math.max(14,tile*.31)}px system-ui`; ctx.textAlign='left'; ctx.fillText(`Level ${state.level}   Moves ${state.moves}   ${state.grid_size}×${state.grid_size}   ${threat}${state.has_treasure ? '   Treasure ✓' : ''}`,left,top-Math.max(30,tile*.48));
    ...
    const treasureVisible = state.debug || state.status !== 'playing' || has(state.visited,...state.treasure) || Math.max(Math.abs(state.treasure[0]-state.player[0]),Math.abs(state.treasure[1]-state.player[1])) <= 2;
    ...
    for(let y=0;y<state.grid_size;y++) for(let x=0;x<state.grid_size;x++) { const seen=state.debug||has(state.visited,x,y)||Math.max(Math.abs(x-state.player[0]),Math.abs(y-state.player[1]))<=2||state.status!=='playing'; if(!seen&&!(state.dragon_awake&&x===state.dragon[0]&&y===state.dragon[1])) sprite('fog',left+x*tile,top+y*tile,tile); }
    ...
    if (state.phantom) { const [x,y] = state.phantom.position, ghostX=left+(x+.5)*tile, ghostY=top+(y+.5)*tile, color=state.phantom.hard_mode ? 'rgba(129,220,255,.75)' : 'rgba(190,145,255,.75)'; ctx.fillStyle=color; ctx.beginPath(); ctx.arc(ghostX,ghostY,tile*.28,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#fff'; ctx.font=`800 ${Math.max(10,tile*.22)}px system-ui`; ctx.textAlign='center'; ctx.fillText(state.phantom.hard_mode ? 'H' : 'N',ghostX,ghostY+tile*.08); }
  ...
  async function showLeaderboard() { leaderboardBody.replaceChildren(); const loading = document.createElement('tr'); loading.innerHTML = '<td colspan="5" class="ranking-empty">Loading rankings…</td>'; leaderboardBody.append(loading); leaderboard.hidden = false; try { const response = await fetch('/api/leaderboard'); const {players} = await response.json(); leaderboardBody.replaceChildren(); if (!players.length) { const empty = document.createElement('tr'); empty.innerHTML = '<td colspan="5" class="ranking-empty">No completed runs yet.</td>'; leaderboardBody.append(empty); return; } players.forEach((player, index) => { const row = document.createElement('tr'); [index + 1, player.username, player.wins, player.best_win_moves ?? '—', player.highest_win_streak].forEach(value => { const cell = document.createElement('td'); cell.textContent = value; row.append(cell); }); leaderboardBody.append(row); }); } catch { leaderboardBody.replaceChildren(); const error = document.createElement('tr'); error.innerHTML = '<td colspan="5" class="ranking-empty">Rankings are unavailable right now.</td>'; leaderboardBody.append(error); } }
  document.querySelector('#leaderboard-toggle').addEventListener('click',showLeaderboard);
  document.querySelector('#leaderboard-close').addEventListener('click',()=>{ leaderboard.hidden=true; });
  profileButton.addEventListener('click',()=>{ showMainMenu(); usernameInput.focus(); usernameInput.select(); });
  addEventListener('keydown',event=>{ if (event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement || event.target.isContentEditable) return; const direction=keys[event.key];if(direction){event.preventDefault();move(direction);}}); canvas.addEventListener('pointerdown',event=>{swipe=[event.clientX,event.clientY];}); canvas.addEventListener('pointerup',event=>{if(!swipe)return;const dx=event.clientX-swipe[0],dy=event.clientY-swipe[1];swipe=null;if(Math.max(Math.abs(dx),Math.abs(dy))>25)move(Math.abs(dx)>Math.abs(dy)?(dx>0?'right':'left'):(dy>0?'down':'up'));}); new ResizeObserver(resize).observe(canvas);
})();
